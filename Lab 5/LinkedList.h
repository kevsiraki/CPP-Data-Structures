...
public:
	...
	bool contains(const ItemType& newEntry) const; //2
	ItemType findLargest() const; //3
	ItemType getLast() const; //4
	vector<ItemType> toVector() const; //to print the list...
}; // end LinkedList
...
{
	assert(getLength() > 0); //make sure list is not empty.
	Node<ItemType>* curPtr = headPtr;
	ItemType largest = curPtr->getItem(); //start at head
	curPtr = curPtr->getNext();
	while (curPtr != nullptr)
	{
		if (curPtr->getItem() > largest) //if the next item is larger than the current largest item
			largest = curPtr->getItem(); //largest is now that item
		curPtr = curPtr->getNext(); //else advance
	}
	return largest;
}
...
{
	assert(getLength() > 0); //make sure list is not empty.
	return tailPtr->getItem(); //return tail or last item...
}
